...
Inductive PremolarCount : Type := P1 | P2.
Inductive BaculumShape : Type := BacSimple | BacBifid | BacComplex | BacReduced.
Inductive RostrumLength : Type := RostrumShort | RostrumMedium | RostrumLong | RostrumElongate.
Inductive AuditoryBullae : Type := BullaeSmall | BullaeMedium | BullaeLarge | BullaeInflated.
Inductive InterorbitalWidth : Type := IONarrow | IOMedium | IOWide.
Inductive StripePattern : Type := NoStripes | DorsalStripe | LateralStripes | FullStripes.
Inductive MammaeCount : Type := M8 | M10 | M12.
Inductive TailRatio : Type := TRShort | TRMedium | TRLong | TRVeryLong.
...
Record Morphology : Type := {
  ...
  premolars : PremolarCount;
  baculum : BaculumShape;
  rostrum : RostrumLength;
  bullae : AuditoryBullae;
  interorbital : InterorbitalWidth;
  mammae : MammaeCount;
...
Definition flying_squirrel_morph : Morphology :=
  {| body_size := Medium; tail_type := Flat; habitat := Gliding;
     ...
     has_ear_tufts := false; stripe_pattern := NoStripes;
     premolars := P2; baculum := BacSimple; rostrum := RostrumMedium;
     bullae := BullaeMedium; interorbital := IOMedium;
     mammae := M8; tail_ratio := TRLong |}.
...
Definition tree_squirrel_morph : Morphology :=
  {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
     ...
     has_ear_tufts := false; stripe_pattern := NoStripes;
     premolars := P1; baculum := BacSimple; rostrum := RostrumMedium;
     bullae := BullaeMedium; interorbital := IOMedium;
     mammae := M8; tail_ratio := TRLong |}.
...
Definition morphology_of (g : Genus) : Morphology :=
  ...
  | Ratufa => {| body_size := Giant; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacComplex;
      rostrum := RostrumLong; bullae := BullaeLarge; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Sciurillus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Microsciurus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Rheithrosciurus => {| body_size := Large; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := true;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacComplex;
      rostrum := RostrumLong; bullae := BullaeMedium; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Sciurus => {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := true;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Syntheosciurus => {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Tamiasciurus => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := true;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRLong |}
  | Aeretes => {| body_size := Medium; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Aeromys => {| body_size := Large; tail_type := Flat; habitat := Gliding;
      ...
      rostrum := RostrumLong; bullae := BullaeLarge; interorbital := IOWide;
      mammae := M8; tail_ratio := TRLong |}
  | Belomys => {| body_size := Small; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Biswamoyopterus => {| body_size := Large; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacComplex;
      rostrum := RostrumLong; bullae := BullaeInflated; interorbital := IOWide;
      mammae := M8; tail_ratio := TRLong |}
  | Eoglaucomys => {| body_size := Medium; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Eupetaurus => {| body_size := Giant; tail_type := Furred; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := true;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacComplex;
      rostrum := RostrumElongate; bullae := BullaeInflated; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Glaucomys => {| body_size := Small; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumShort; bullae := BullaeLarge; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Hylopetes => {| body_size := Small; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  ...
  | Petaurillus => {| body_size := Tiny; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacReduced;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRShort |}
  | Petaurista => {| body_size := Giant; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacComplex;
      rostrum := RostrumLong; bullae := BullaeLarge; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Petinomys => {| body_size := Small; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Pteromys => {| body_size := Medium; tail_type := Flat; habitat := Gliding;
      cheek_pouches := Absent; has_patagium := true; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeInflated; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  ...
  | Trogopterus => {| body_size := Medium; tail_type := Flat; habitat := Gliding;
      ...
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Callosciurus => {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P1; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Dremomys => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Exilisciurus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := DorsalStripe; premolars := P1; baculum := BacReduced;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Funambulus => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := FullStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Glyphotes => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      ...
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Hyosciurus => {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacComplex;
      rostrum := RostrumElongate; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Lariscus => {| body_size := Small; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := FullStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Menetes => {| body_size := Small; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := FullStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Nannosciurus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacReduced;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRShort |}
  | Prosciurillus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := DorsalStripe; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Rhinosciurus => {| body_size := Medium; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacComplex;
      rostrum := RostrumElongate; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRMedium |}
  | Rubrisciurus => {| body_size := Large; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacBifid;
      rostrum := RostrumLong; bullae := BullaeLarge; interorbital := IOWide;
      mammae := M8; tail_ratio := TRLong |}
  | Sundasciurus => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacBifid;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Tamiops => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := FullStripes; premolars := P1; baculum := BacReduced;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRMedium |}
  | Atlantoxerus => {| body_size := Medium; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRLong |}
  | Spermophilopsis => {| body_size := Medium; tail_type := Thin; habitat := Fossorial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumLong; bullae := BullaeInflated; interorbital := IOWide;
      mammae := M10; tail_ratio := TRShort |}
  | Xerus => {| body_size := Medium; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRMedium |}
  | Epixerus => {| body_size := Large; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacBifid;
      rostrum := RostrumLong; bullae := BullaeMedium; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Funisciurus => {| body_size := Small; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRLong |}
  | Heliosciurus => {| body_size := Medium; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M8; tail_ratio := TRLong |}
  | Myosciurus => {| body_size := Tiny; tail_type := Bushy; habitat := Arboreal;
      cheek_pouches := Absent; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P1; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeSmall; interorbital := IONarrow;
      mammae := M8; tail_ratio := TRShort |}
  ...
  | Protoxerus => {| body_size := Large; tail_type := Bushy; habitat := Arboreal;
      ...
      rostrum := RostrumLong; bullae := BullaeLarge; interorbital := IOWide;
      mammae := M8; tail_ratio := TRVeryLong |}
  | Ammospermophilus => {| body_size := Small; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeLarge; interorbital := IONarrow;
      mammae := M10; tail_ratio := TRShort |}
  | Callospermophilus => {| body_size := Medium; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := FullStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRMedium |}
  | Cynomys => {| body_size := Medium; tail_type := Thin; habitat := Fossorial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacComplex;
      rostrum := RostrumShort; bullae := BullaeInflated; interorbital := IOWide;
      mammae := M10; tail_ratio := TRShort |}
  | Ictidomys => {| body_size := Small; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := DorsalStripe; premolars := P2; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeLarge; interorbital := IONarrow;
      mammae := M10; tail_ratio := TRShort |}
  | Marmota => {| body_size := Giant; tail_type := Bushy; habitat := Fossorial;
      ...
      rostrum := RostrumLong; bullae := BullaeInflated; interorbital := IOWide;
      mammae := M10; tail_ratio := TRShort |}
  | Notocitellus => {| body_size := Medium; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := DorsalStripe; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRShort |}
  | Otospermophilus => {| body_size := Medium; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRMedium |}
  | Poliocitellus => {| body_size := Medium; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeMedium; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRShort |}
  | Sciurotamias => {| body_size := Medium; tail_type := Bushy; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := LateralStripes; premolars := P2; baculum := BacBifid;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRMedium |}
  | Spermophilus => {| body_size := Medium; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRShort |}
  | Tamias => {| body_size := Small; tail_type := Bushy; habitat := Terrestrial;
      ...
      rostrum := RostrumShort; bullae := BullaeLarge; interorbital := IONarrow;
      mammae := M10; tail_ratio := TRMedium |}
  | Urocitellus => {| body_size := Medium; tail_type := Thin; habitat := Fossorial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := NoStripes; premolars := P2; baculum := BacSimple;
      rostrum := RostrumMedium; bullae := BullaeLarge; interorbital := IOMedium;
      mammae := M10; tail_ratio := TRShort |}
  | Xerospermophilus => {| body_size := Small; tail_type := Thin; habitat := Terrestrial;
      cheek_pouches := Present; has_patagium := false; has_ear_tufts := false;
      stripe_pattern := DorsalStripe; premolars := P2; baculum := BacSimple;
      rostrum := RostrumShort; bullae := BullaeMedium; interorbital := IONarrow;
      mammae := M10; tail_ratio := TRShort |}
